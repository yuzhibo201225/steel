...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train YOLO baseline on NEU-DET fold")
...
def smoke_metrics(seed: int, fold_id: int) -> tuple[float, float, dict[str, float]]:
    rng = random.Random(seed * 200 + fold_id)
    ap50 = round(0.68 + rng.random() * 0.2, 4)
    map_50_95 = round(ap50 - (0.16 + rng.random() * 0.05), 4)
    per_class = {name: round(max(0.1, ap50 - rng.random() * 0.25), 4) for name in DEFAULT_CLASSES}
...
def main() -> None:
    ...
    if not args.smoke_test:
        ...
        results = model.train(
            ...
            name=f"yolo/fold_{args.fold_id}",
        )
